...
public enum Settings {
    ...
    SIGIS_EMAIL_ADMINADDRESS(
        "sigis.email.adminaddress",
        "apteagrouptest@gmail.com"
    ),
    IRSN_WEBSITE_URL("irsn.website.url", "http://www.irsn.fr/"),
    SIGIS_WEBSITE_URL(
        "sigis.website.url",
        "http://localhost:8080/sigis-web-fo/"
    ),
    ...
    TIME_RECALL_MONTHS_BALANCE_SUPPLIERS(
        "sigis.time.balance.suppliers.months",
        "3"
    ),
    TIME_ALLOWED_EMAILS_CONFIRMATION("sigis.time.emails.confirmation", "30"),
    TIME_AFTER_WHICH_CONTACT_CONSIDERED_POSTED(
        "sigis.time.contacts.month.posted",
        "1"
    ),
    TIME_MOVES_NO_FORM_VALIDITY("sigis.time.moves.no.form.expire", "6"),
    TIME_DEFAULT_REPLY_EXPECTED_DAYS(
        "sigis.time.contacts.reply.expected",
        "15"
    ),
    FORM_NUMBER_LIMIT_DISTINCTION_FO_BO(
        "sigis.form.number.distinction.fobo",
        "2000000"
    ),
    ...
    DIGITAL_SIGNATURE_ID_APPLICATION(
        "digital.signature.id.application",
        "ZZDEMAV1"
    ),
    DIGITAL_SIGNATURE_ID_APPLICATION_SERVER(
        "digital.signature.id.application.server",
        "DEMO"
    ),
    ...
    DIGITAL_SIGNATURE_SUBMIT_URL(
        "digital.signature.url",
        "https://keynectis.kwebsign.net/QS/Page1V2"
    ),
    ...
    static final Pattern extractCustomLevel = Pattern.compile("log.level.(.*)");
    static final Pattern extractCustomAppender = Pattern.compile(
        "log.appender.(.*)"
    );
    ...
    private static void showErrorMessage() {
        StringBuilder message = new StringBuilder(
            "La variable d'environnement SIGIS_SETTINGS_FILE doit référencer un fichier contenant les paramètres suivants (les valeurs par défaut sont indiquées ici) : "
        );
    ...
    public static Map<String, String> listCustomLoggingSettings() {
        ...
        for (String key : customLevels.keySet()) {
            result.put(
                String.format("custom log level (log.level.%s)", key),
                customLevels.get(key).toString()
            );
        }
        for (String key : customAppenders.keySet()) {
            result.put(
                String.format("custom log appender (log.appender.%s)", key),
                customAppenders.get(key)
            );
        }
        return result;
    }
